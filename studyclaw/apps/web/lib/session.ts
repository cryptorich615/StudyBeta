...
export function isOnboardingComplete(session?: StoredSession | null): boolean {
  const resolved = session ?? readStoredSession();
  if (!resolved?.user?.id) {
    return false;
  }
  ...
  return Boolean(
    resolved.onboardingComplete ??
    resolved.onboarding_complete ??
    (resolved.user.agent_type && resolved.user.agent_type !== 'custom')
  );
}
