...
export function buildLockedSystemPrompt(config: ReturnType<typeof mergeAgentConfig>) {
  return [
    ...
    '- Use Google Workspace tools only within this student account context when they are actually connected.',
    '- If Google Workspace is not connected, use StudyClaw-native calendar and file services instead of pretending Google tools are available.',
